...
void Game::loadSettings(Renderer * myRenderer)
{
	...
	myRenderer->LoadTextureText("SUNSPIRE", { "GAME OVER","gameOverID",white });
	myRenderer->LoadTextureText("SUNSPIRE", { "ENTER WINNER'S NAME","winnerNameID",white });
	//******************************LOAD_POWER_UPS*******************
...
void Game::Update(Controller * inputs)
{
	...
	case GAME_OVER:
		sceneName = RANKING;
		newMaxPuntuation = (player1->score >= player2->score) ? player1->score : player2->score;
		std::cout << "EnterYourNameWinner:" << std::endl;
		std::cin >> nameWinerPlayer;
...
void Game::Draw(Renderer * myRenderer)
{
		...
		//******************************TRANSITIONS*********************************
	{
		...
		else if (sceneState == GAME_OVER) 
		{
			myRenderer->PushImage("bTrans", { 0,0, SCREEN_WIDTH, SCREEN_HEIGHT });
			myRenderer->PushImage("gameOverID", { (SCREEN_WIDTH / 2) - 150,(SCREEN_HEIGHT / 4) - 75,300,150 });
			myRenderer->PushImage("winnerNameID", { (SCREEN_WIDTH / 2) - 225,(SCREEN_HEIGHT / 2) - 50,450,100 });
		}
	}
	//****************************************************************************
